...
namespace Connect.IdeaBox.IdeaBox.Common
{
  public class ContextSecurity
  {
    ...
    public ContextSecurity(ModuleInfo objModule, UserInfo user)
    {
      ...
      if (user.IsSuperUser)
      {
        CanSubmit = CanVote = CanView = CanEdit = IsAdmin = true;
      }
      else
      {
        ...
        if (IsAdmin)
        {
          CanSubmit = CanVote = CanView = CanEdit = true;
        }
        else
        {
          ...
          CanSubmit = ModulePermissionController.HasModulePermission(objModule.ModulePermissions, "CANSUBMIT");
          CanVote = ModulePermissionController.HasModulePermission(objModule.ModulePermissions, "CANVOTE");
        }
      }
    }
    #endregion
